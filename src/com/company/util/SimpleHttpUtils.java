...
public class SimpleHttpUtils {
    ...
    public static String sendRequest(String url, String method, Map<String, String> headers, InputStream bodyStream, File saveToFile) throws Exception {
        ...
        try {
            ...
            if (saveToFile != null) {
                handleResponseBodyToFile(in, saveToFile, contentLength);
    ...
    //添加最大长度——hzg
    private static void handleResponseBodyToFile(InputStream in, File saveToFile, long contentLength) throws Exception {
        OutputStream out = null;
        try {
            out = new FileOutputStream(saveToFile);
            //System.out.print(saveToFile);
            copyStreamAndClose(in, out, contentLength);
        } finally {
            closeStream(out);
        }
    }
    ...
    //添加一个参数，文件总大小
    private static void copyStreamAndClose(InputStream in, OutputStream out, long contentLength) {
        try {
            byte[] buf = new byte[1024];
            int len = -1;
            long total = 0;
            //生成一个任务就产生一个进度条
            Downloading downloading = GetMainUIComponent.getDownloading();
            Downloading.Item item = downloading.new Item(1);
